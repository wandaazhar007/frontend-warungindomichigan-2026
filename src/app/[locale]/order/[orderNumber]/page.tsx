...
function StatusTimeline({ current }: { current: OrderStatus }) {
  ...
  const tStatus = useTranslations('Orders.OrderStatusTimeline');
  ...
  if (cancelled) {
    return (
      <div className="flex items-center gap-3 p-4 bg-gray-50 rounded-xl text-sm text-gray-500">
        ...
        {current === 'REFUNDED' ? t('orderRefunded') : t('orderCancelled')}
      </div>
    );
  }
  ...
  if (error || !order) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-4 text-center px-4">
        ...
        <Button asChild variant="outline"><Link href="/products">{t('backToShopping')}</Link></Button>
      </div>
    );
  }
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-10 space-y-8">
        ...
        <div className="text-center space-y-3">
          ...
          <h1 className="font-display text-2xl sm:text-3xl font-bold text-gray-900">
            {isPaid ? t('thankYou') : t('orderReceived')}
          </h1>
        ...
        {isPaid && (
          <div className="bg-green-50 border border-green-200 rounded-2xl p-5 flex flex-col sm:flex-row items-start sm:items-center gap-4">
            ...
            <div className="flex-1">
              <p className="font-semibold text-green-800 text-sm">{t('paymentConfirmed')}</p>
              <p className="text-green-700 text-sm mt-0.5">
                {t.rich('paymentConfirmedDescription', {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white rounded-xl border border-gray-100 p-5">
              <h2 className="font-display font-700 text-gray-900 mb-4">{t('orderItems')}</h2>
            ...
            {/* Status timeline */}
            <div className="bg-white rounded-xl border border-gray-100 p-5">
              <h2 className="font-display font-700 text-gray-900 mb-5">{t('orderStatus')}</h2>
            ...
            {order.trackingNumber && (
              <div className="bg-white rounded-xl border border-gray-100 p-5">
                <h2 className="font-display font-700 text-gray-900 mb-3">{t('shippingInfo')}</h2>
                  ...
                  {order.trackingUrl && (
                    <Button variant="outline" size="sm" asChild>
                      <a href={order.trackingUrl} target="_blank" rel="noopener noreferrer">
                        {t('trackPackage')} <ExternalLink className="h-3.5 w-3.5 ml-1.5" />
                      </a>
                    </Button>
                  )}
                </div>
          ...
          <div className="space-y-5">
            ...
            <div className="bg-white rounded-xl border border-gray-100 p-5 space-y-3">
              <h2 className="font-display font-700 text-gray-900 text-sm">{t('status')}</h2>
            ...
            <div className="bg-white rounded-xl border border-gray-100 p-5">
              <h2 className="font-display font-700 text-gray-900 text-sm mb-3 flex items-center gap-2">
                <MapPin className="h-4 w-4 text-red-400" /> {t('shippingAddress')}
              </h2>
            ...
            <div className="bg-white rounded-xl border border-gray-100 p-5">
              <h2 className="font-display font-700 text-gray-900 text-sm mb-3 flex items-center gap-2">
                <CreditCard className="h-4 w-4 text-red-400" /> {t('payment')}
              </h2>
              <p className="text-sm text-gray-600">
                {isPaid
                  ? `${t('paid')}${order.paidAt ? ` ${t('on')} ${new Date(order.paidAt).toLocaleDateString(dateLocale, { day: 'numeric', month: 'long', year: 'numeric' })}` : ''}`
                  : tPayment(order.paymentStatus)}
              </p>
            </div>
