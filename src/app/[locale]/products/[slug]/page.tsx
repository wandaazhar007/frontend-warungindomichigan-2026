  ...
  if (!product) {
    return (
      <div className="container-wim py-20 text-center">
        ...
        <h2 className="font-display text-2xl font-bold text-gray-900 mb-2">{t('notFound')}</h2>
    ...
    <div className="bg-background min-h-screen">
      <div className="container-wim py-6 sm:py-10">
        ...
        <nav className="flex items-center gap-1.5 text-xs text-gray-500 mb-6 flex-wrap">
          ...
          <Link href="/products" className="hover:text-primary transition-colors">{t('breadcrumbAllProducts')}</Link>
          <ChevronRight className="h-3 w-3 text-gray-300 shrink-0" />
          ...
          <div>
            ...
            {isOutOfStock ? (
              ...
              <div className="bg-yellow-50 border border-yellow-200 text-yellow-700 text-sm font-semibold px-4 py-2 rounded-xl inline-block mb-5">
                ⚠️ {t('onlyLeftInStock', { count: product.stock })}
              </div>
            ) : (
            ...
            {!isOutOfStock && (
              <div className="mb-5">
                <p className="text-sm font-semibold text-gray-700 mb-2">{t('quantity')}</p>
            ...
            <div className="flex flex-col sm:flex-row gap-3 mb-6">
              ...
              >
                ...
                {cartItem ? t('addedToCart') : `${t('addToCart')} — ${formatPrice(price * qty)}`}
              </Button>
              <Button
                size="lg"
                variant="outline"
            ...
            <div className="flex items-center gap-2 text-sm text-gray-600 bg-white border border-border rounded-xl px-4 py-3">
              ...
              <span>{t.rich('freeDeliveryNote', { amount: (chunks) => <span className="font-semibold">{chunks}</span> })}</span>
            </div>

            {/* Description */}
            {product.description && (
              <div className="mt-8">
                <h3 className="font-display font-semibold text-gray-900 mb-3">{t('aboutProduct')}</h3>
